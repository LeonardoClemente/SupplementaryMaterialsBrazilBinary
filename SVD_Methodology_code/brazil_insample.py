...
def SVDC_insample_heatmapgen(df, period_of_interest, leave_last_out=False,first_year_insample = 2001,last_year_insample = 2006,\
    ...
    distance_grids = {}
    in_sample_years = list(range(first_year_insample, last_year_insample+1))
    if leave_last_out:
        in_sample_years.pop()

    for y in in_sample_years:
        distance_grids[y] = copy.copy(distance_grid)
    ...
    dates_within_poi=[]
    for d in t0_vector:
        if '{0}'.format(last_year_insample) + d[4:] in df[period_of_interest[0]:period_of_interest[1]].index:
    ...
    for i, p in enumerate(p_vector):
        ...
        for j, t0 in enumerate(t0_vector):
            ...
            if verbose:
                print('Reshaping data with upper bound : {0}'.format(period_of_interest[0]))
                time.sleep(10)
            ...
            if X is not None:
                ...
                if verbose:
                    ...
                    print('prediction_year:{0}, column:{1}'.format(prediction_year, X_predict_column))
                    print('Training years:{0}, columns:{1}'.format(in_sample_years, X_train_columns))
                    print('Train_x : {0}, predict_x : {1}'.format(X_train, X_predict))
                    print('DICT: {0}'.format(epidemic_classification_dict))
                    print('Y_predict : {0}, prediction_year:{1}'.format(Y_predict, prediction_year))
                    print('Y_train : {0}, in_sample_years:{1}'.format(Y_train, in_sample_years))
                    time.sleep(100)
                # Perform svd
                ...
                if leave_last_out:
                    ...
                    classifier_dataset_train = classifier_dataset
                ...
                training_pred = mod.predict(classifier_dataset_train)
                score = np.equal(training_pred.reshape(-1,1), Y_train.reshape(-1,1))
                for k,y in enumerate(in_sample_years):
                    distance_grids[y][i,j] = score[k]
                distance_grid[i,j] = np.sum(score)

            else:
                distance_grid[i,j] = -1
                for k,y in enumerate(in_sample_years):
                    distance_grids[y][i,j] = -1
    bar.finish()
    return distance_grid, distance_grids


# Function Params
leave_last_out = True
verbose = False #Prints data flow
first_year_insample = 2001
last_year_insample = 2005
last_years = range(2005,2017)
heatmap_version = 6
modes = [1]
...
# Files and folders
heatmap_path = '/Users/leonardo/Desktop/FORECASTING/STOLERMAN/BRAZIL_heatmap_dicts/'
csv_data_path = '/Users/leonardo/Desktop/FORECASTING/STOLERMAN/SVDC_DATA_allvars'

brazil_stations = ['BeloHorizonte', 'Manaus', 'Aracaju', 'Barueri', 'Sertaozinho'] #'BeloHorizonte' #'Manaus', 'Aracaju', 'Barueri', 'Sertaozinho',


all_stations=[ 'Aracaju', 'SaoGoncalo', 'SantaCruz', 'JuazeirodoNorte', 'JiParana', 'Rondonopolis',\
...
for l in last_years:
    for station in all_stations:
        ...
        print('Computing heatmap loop for {0} and years {1}-{2}.'.format(station, first_year_insample, l))
        print('modes used: {0} average days between peaks added: {1}'.format(modes, add_peaks))
        print('Params: leave_last_out : {0}'.format(leave_last_out))
        print('Params: variables {0}'.format(variables))
        print('classifier: {0}'.format(classifier))
        time.sleep(3)
        heatmap, heatmaps = SVDC_insample_heatmapgen(full_df, leave_last_out=leave_last_out, \
                        first_year_insample = first_year_insample,\
                          last_year_insample = l,\
                           period_of_interest=['{0}-01-01'.format(l), '{0}-05-31'.format(l)],\
                            epidemic_classification_dict=epidemic_classification_dict, training_year_window='ALL',\
                            t0_vector=t0_vector, p_vector=p_vector, classifier=classifier, modes=[1],\
                            variables=variables, verbose=verbose, add_peaks=add_peaks, add_runoff_binary=add_runoff_binary)

        if leave_last_out:
            pickle.dump([heatmap, t0_vector, p_vector], open("{0}/{1}/accumulated_v{2}_{3}_{4}_preproc.p".format(heatmap_path, station,\
                                                                        heatmap_version, first_year_insample, l), "wb" ))
            pickle.dump([heatmaps, t0_vector, p_vector], open("{0}/{1}/separated_v{2}_{3}_{4})_preproc.p".format(heatmap_path, station,\
                                                                    heatmap_version, first_year_insample, l), "wb" ))
        else:
            pickle.dump([heatmap, t0_vector, p_vector], open("{0}/{1}/accumulated_v{2}_{3}_{4}.p".format(heatmap_path, station,\
                                                                        heatmap_version, first_year_insample, l), "wb" ))
            pickle.dump([heatmaps, t0_vector, p_vector], open("{0}/{1}/separated_v{2}_{3}_{4}.p".format(heatmap_path, station,\
                                                                    heatmap_version, first_year_insample, l), "wb" ))
