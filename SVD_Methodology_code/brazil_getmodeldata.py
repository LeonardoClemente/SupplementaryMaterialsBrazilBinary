...
def SVDC_deploy(df, period_of_interest, variables=['precip', 'temp'], add_runoff_binary=False, prediction_year=2012, epidemic_classification_dict=None, first_training_year=2000, t0_vector=None, p_vector=None, classifier='forest', modes=[0,1], decision_map=None, decision_coordinates=None, decision_values=None, clustering=True, verbose=False, tiebreaker=True):
    ...
    return final_decision, cluster_decisions, cluster_weights, distance_grid, fig, votes_favor, votes_against, total_votes
...
## MODEL SELECTION
#first_year_insample = 2006
#last_year_insample = 2011
...
for last_year_insample in range(2011,2017):
    first_year_insample = last_year_insample - 5

    df = collections.OrderedDict()
    accuracy = collections.OrderedDict()
    votes_favor_df = collections.OrderedDict()
    total_votes_df = collections.OrderedDict()
    for station in brazil_stations:
        ...
        data = pickle.load(open(\
                "/Users/leonardo/Desktop/FORECASTING/STOLERMAN/{3}/heatmap_dicts/{0}/v6_ALL_YEARS_combined.p".format(station, mod, version, experiment_folder), "rb" )) #v{2}_ALL_YEARS.p".format(station, mod, version), "rb" ))
    ...
    #            "/Users/leonardo/Desktop/FORECASTING/STOLERMAN/{3}/heatmap_dicts/{0}/accumulated_v6_2001_2005.p".format(station, mod, version, experiment_folder), "rb" )) #v{2}_ALL_YEARS.p".format(station, mod, version), "rb" ))
        ...
        for prediction_year in range(first_year_insample, last_year_insample+1):
            ...
            for y in range(first_year_insample, last_year_insample+1):
            ...
            final_decision, cluster_decisions, cluster_weights, distance_grid, fig, vf, va, tv = SVDC_deploy(full_df, period_of_interest=['{0}-03-01'.format(prediction_year), \
        ...
        decision_score = np.round(decision_score)
        df[station] = decision_score
        accuracy[station] = np.sum(np.equal(decision_score, 0))/len(decision_score)
        votes_favor_df[station] = np.array(votes_favor)
        total_votes_df[station] = np.array(total_votes)


    votes_favor_df = pd.DataFrame(votes_favor_df, index=list(range(first_year_insample,last_year_insample+1)))
    total_votes_df = pd.DataFrame(total_votes_df, index=list(range(first_year_insample,last_year_insample+1)))
    votes_favor_df.to_csv('/Users/leonardo/Desktop/FORECASTING/STOLERMAN/ENSAMBLE/First/votes_favor_{0}.csv'.format(last_year_insample+1))
    total_votes_df.to_csv('/Users/leonardo/Desktop/FORECASTING/STOLERMAN/ENSAMBLE/First/total_votes_{0}.csv'.format(last_year_insample+1))
    df = pd.DataFrame(df, index=list(range(first_year_insample,last_year_insample+1)))
    df.to_csv('/Users/leonardo/Desktop/FORECASTING/STOLERMAN/ENSAMBLE/First/score_{0}.csv'.format(last_year_insample+1))
    ax = sns.heatmap(df.transpose(), linewidths=.5,vmin=-2, vmax=2, cmap='coolwarm')
    plt.subplots_adjust(left=0.29, bottom=0.11, right=1.0, top=.88, wspace=.20, hspace=.20)
    plt.title('{0}-{1} heatmap out of sample predictions'.format(first_year_insample, last_year_insample))
    plt.savefig('/Users/leonardo/Desktop/FORECASTING/STOLERMAN/ENSAMBLE/First/heatmap_{0}.png'.format(last_year_insample+1), format='png')
    plt.figure()
    ax2 = sns.barplot(data=pd.DataFrame(accuracy, index=['Accuracy']))
    plt.savefig('/Users/leonardo/Desktop/FORECASTING/STOLERMAN/ENSAMBLE/First/accuracy_{0}.png'.format(last_year_insample+1), format='png')
    plt.close()
